...
- (void)initValues
{
    ...
    //创建顶部可滑动的tab
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width,IS_IOS7?kHeightOfTopScrollView:44)];
    ...
    _topScrollView.backgroundColor =  [SUNSlideSwitchView colorFromHexRGB:@"f5f5f5"];
    [self addSubview:_topScrollView];
    _userSelectedChannelID = 100;
    
    //创建主滚动视图
    _rootScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, IS_IOS7?kHeightOfTopScrollView:kHeightOfTopScrollView -20, self.bounds.size.width, self.bounds.size.height + kHeightOfTopScrollView)];
    ...
    if (_isBuildUI) {
        ...
        if (self.rigthSideButton.bounds.size.width > 0) {
            ...
            _topScrollView.frame = CGRectMake(0, 0,
                                              self.bounds.size.width - self.rigthSideButton.bounds.size.width, IS_IOS7?kHeightOfTopScrollView:44);
        }
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        CGSize textSize = [vc.title sizeWithFont:[UIFont systemFontOfSize:kFontSizeOfTabButton]
                               constrainedToSize:CGSizeMake(_topScrollView.bounds.size.width, IS_IOS7?kHeightOfTopScrollView:44)
                                   lineBreakMode:NSLineBreakByTruncatingTail];
        //累计每个tab文字的长度
        topScrollViewContentWidth += (kWidthOfButtonMargin+textSize.width + 15);
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset,IS_IOS7?-10:0,
                                    textSize.width + 15, IS_IOS7?kHeightOfTopScrollView:44)];
        //计算下一个tab的x偏移量
        xOffset += textSize.width + kWidthOfButtonMargin + 15;
        
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin -2, -5, textSize.width + 20 , _shadowImage.size.height);
            button.selected = YES;
        }
...
//          button.titleLabel.font =  [UIFont fontWithName:@"TrebuchetMS-Bold" size:kFontSizeOfTabButton];
        }
    ...
    //设置顶部滚动视图的内容总尺寸
    _topScrollView.contentSize = CGSizeMake(topScrollViewContentWidth , (IS_IOS7?kHeightOfTopScrollView:44)-20);
}
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x - 4, -5, sender.frame.size.width + 5, _shadowImage.size.height)];
...
- (void)adjustScrollViewContentX:(UIButton *)sender
{
    ...
    if (sender.frame.origin.x - _topScrollView.contentOffset.x > self.bounds.size.width - (kWidthOfButtonMargin+sender.bounds.size.width)) {
        //向左滚动视图，显示完整tab文字
        [_topScrollView setContentOffset:CGPointMake(sender.frame.origin.x - (_topScrollView.bounds.size.width- (kWidthOfButtonMargin+sender.bounds.size.width)), IS_IOS7?-17:0)  animated:YES];
    }
    ...
    if (sender.frame.origin.x - _topScrollView.contentOffset.x < kWidthOfButtonMargin) {
        ...
        [_topScrollView setContentOffset:CGPointMake(sender.frame.origin.x - kWidthOfButtonMargin,IS_IOS7?-17:0)  animated:YES];
    }
}
...
//        }
//    } else
        if(_rootScrollView.contentOffset.x >= _rootScrollView.contentSize.width - _rootScrollView.bounds.size.width) {
        if (self.slideSwitchViewDelegate
